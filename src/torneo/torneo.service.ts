...
export class TorneoService {
  ...
  private borrarFlyerAnterior(flyerUrl?: string | null) {
    ...
    const rutaRelativa = flyerUrl.replace(/^\/+/, '');
    ...
    try {
      ...
      console.warn('No se pudo borrar el flyer anterior:', error);
    }
  }
  ...
  async update(
    ...
    if (file) {
      this.borrarFlyerAnterior(torneo.flyer_url);
      torneo.flyer_url = `/uploads/torneos/${file.filename}`;
    }
